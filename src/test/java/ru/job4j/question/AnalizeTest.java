...
class AnalizeTest {
    ...
    @Test
    void whenDeletedAndChangedAndAdded() {
        User u1 = new User(1, "A");
        User u2 = new User(2, "B");
        User u3 = new User(3, "C");
        Set<User> previous = Set.of(u1, u2, u3);
        Set<User> current = Set.of(new User(4, "D"), new User(3, "CCCC"));
        assertThat(Analize.diff(previous, current)).isEqualTo(new Info(1, 1, 2));
    }
}